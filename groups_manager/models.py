...
class MemberMixin(MemberRelationsMixin, models.Model):
    ...
    email = models.EmailField(_('email'), max_length=255, default='', blank=True)
    ...
    def save(self, *args, **kwargs):
        if not self.username:
            self.username = slugify(self.full_name, separator="_")
...
class Group(GroupMixin):
    ...
    group_type = models.ForeignKey(GroupType, null=True, blank=True, on_delete=models.SET_NULL,
                                   related_name='%(app_label)s_%(class)s_set', verbose_name=_('group type'))
    group_entities = models.ManyToManyField(GroupEntity, blank=True,
                                            related_name='%(app_label)s_%(class)s_set', verbose_name=_('group entities'))

    django_group = models.ForeignKey(DjangoGroup, null=True, blank=True, on_delete=models.SET_NULL, verbose_name=_('django group'))
    group_members = models.ManyToManyField(Member, through='GroupMember',
                                           related_name='%(app_label)s_%(class)s_set', verbose_name=_('group members'))
...
class GroupMember(GroupMemberMixin):
    ...
    roles = models.ManyToManyField(GroupMemberRole, blank=True, verbose_name=_('roles'))
    expiration_date = models.DateTimeField(_('expiration date'), null=True, default=None, blank=True)
